...
prove<
  Equals<
    V.MapValidation<[string], string[]>,
    Array<V.Validation<[string], string>>
  >
>("proof");
...
describe("arrayToValidation", () => {
  it("returns an error when input is empty", () => {
    expect(simplify(V.arrayToValidation([], ["error"]))).toEqual(
      simplify(V.Invalid(["error"]))
    );
  });
  it("returns the first element when the input is not empty", () => {
    fc.assert(
      fc.property(
        ...
        (xs: number[]) => {
          expect(simplify(V.arrayToValidation(xs, ["error"]))).toEqual(
            simplify(V.Valid(xs[0]))
          );
        }
      )
    );
...
describe("maybeToValidation", () => {
  it("returns an error when input is empty", () => {
    expect(simplify(V.maybeToValidation(Nothing(), ["error"]))).toEqual(
      simplify(V.Invalid(["error"]))
    );
  });
  it("returns the value when the input is not empty", () => {
    fc.assert(
      fc.property(fc.anything(), a => {
        expect(simplify(V.maybeToValidation(Just(a), ["error"]))).toEqual(
          simplify(V.Valid(a))
        );
      })
    );
...
describe("eitherToValidation", () => {
  it("maps Left -> Invalid", () => {
    fc.assert(
      fc.property(fc.oneof(fc.array(fc.anything()), fc.object()), e => {
        expect(simplify(V.eitherToValidation(Left(e)))).toEqual(
          simplify(V.Invalid(e))
        );
      })
    );
  });
  it("returns the value when the input is not empty", () => {
    fc.assert(
      fc.property(fc.anything(), a => {
        expect(simplify(V.eitherToValidation(Right<any[], any>(a)))).toEqual(
          simplify(V.Valid(a))
        );
      })
    );
    ...
    fc.assert(
      fc.property(
        ...
        (xs: Array<V.Validation<[string], number>>) => {
          expect(V.failures(xs)).toEqual(
            xs.filter(x => x.isInvalid()).map(x => (x as any).failure)
          );
        }
      )
    );
  ...
  it("returns the value of every Invalid", () => {
    fc.assert(
      fc.property(
        ...
        (xs: Array<V.Validation<[string], number>>) => {
          expect(V.successful(xs)).toEqual(
            xs.filter(x => x.isValid()).map(x => (x as any).value)
          );
        }
      )
    );
    ...
    fc.assert(
      fc.property(fc.array(fc.integer()), (xs: number[]) => {
        ...
        expect(resultMapM).toEqual(
          simplify(V.Valid(xs.map(x => x.toString())))
        );
      })
    );
  });
      ...
      fc.property(
        ...
        ([size, empties]) => {
          ...
          expect(resultMapM).toEqual(
            simplify(
              V.Invalid(
                empties.sort().filter((x, i, arr) => arr.indexOf(x) === i)
              )
            )
          );
        }
    ...
    fc.assert(
      fc.property(
        ...
        (a: number, b: boolean, c: string) => {
          expect(
            simplify(V.lift(f, V.Valid(a), V.Valid(b), V.Valid(c)))
    ...
    fc.assert(
      fc.property(
        ...
        (a: number, b: boolean, c: string, empties: number[]) => {
          ...
          expect(
            simplify(V.lift(f, getArg(0, a), getArg(1, b), getArg(2, c)))
          ).toEqual(
            simplify(
              V.Invalid(
                empties.sort().filter((x, i, arr) => arr.indexOf(x) === i)
      ...
      fc.property(
        ...
        (xys: Array<[number, string]>) => {
          expect(
            simplify(
              V.mapAndUnzipWith(
                ([x, y]) => V.Valid([y, x] as [string, number]),
                xys
              )
            )
          ).toEqual(
            simplify(
              V.Valid(unzip(xys.map(([x, y]) => [y, x] as [string, number])))
            )
          );
        }
      )
  ...
  it("is equal to wrap for only Valid results", () => {
    fc.assert(
      fc.property(fc.array(fc.string()), (strs: string[]) => {
        expect(simplify(V.sequence(strs.map(V.Valid)))).toEqual(
          simplify(V.Valid(strs))
        );
      })
    );
  });
        ...
        ([strs, empties]) => {
          ...
          expect(
            simplify(
              V.sequence(
                strs.map(str =>
                  predicate(str) ? V.Valid(str[0]) : V.Invalid([str[1]])
                )
              )
            )
          ).toEqual(
    ...
    fc.assert(
      fc.property(
        ...
        (strs: string[], ns: number[]) => {
          expect(
            simplify(V.zipWithM((str, n) => V.Valid(str.length + n), strs, ns))
          ).toEqual(
            simplify(V.Valid(strs.zipWith((str, n) => str.length + n, ns)))
          );
        }
      )
    );
      ...
      fc.property(arb, ([strs, ns, empties]) => {
        ...
        expect(
          simplify(
            V.zipWithM(
              ([str, i], n) =>
                predicate(i) ? V.Valid(str.length + n) : V.Invalid([i]),
              strs,
              ns
            )
          )
...
describe("IValidation", () => {
  describe("defaultWith", () => {
    it("Returns default when Invalid", () => {
      fc.assert(
        fc.property(fc.string(), s => {
          expect(V.Invalid([s]).defaultWith("foo")).toEqual("foo");
        })
      );
    });
    it("Returns payload when Valid", () => {
      fc.assert(
        fc.property(fc.string(), s => {
          expect(V.Valid(s).defaultWith("foo")).toEqual(s);
        })
      );
    });
  });
  ...
  describe("map", () => {
    it("Passes the payload to the callback", () => {
      fc.assert(
        fc.property(fc.string(), s => {
          V.Valid(s).map(x => expect(x).toEqual(s));
        })
      );
    });
    ...
    it("Skips the callback on empty", () => {
      ...
      expect(
        simplify(
          V.Invalid<[string], string>(["error"]).map(k)
        )
      ).toEqual(simplify(V.Invalid(["error"])));
    });
  });
  ...
  describe("mapError", () => {
    it("Passes the payload to the callback", () => {
      fc.assert(
        fc.property(fc.string(), s => {
          V.Invalid([s]).mapError(x => expect(x).toEqual([s]));
        })
      );
    });
    it("Wraps the value returned by the callback", () => {
      ...
      fc.assert(
        fc.property(fc.string(), s => {
          expect(simplify(V.Invalid([s]).mapError(k))).toEqual(
            simplify(V.Invalid(k([s])))
          );
        })
      );
    });
    it("Skips the callback on empty", () => {
      const k = (s: string[]) => [s.length];
      expect(simplify(V.Valid<[string], string>("foo").mapError(k))).toEqual(
        simplify(V.Valid("foo"))
      );
    });
  });
  ...
  describe("matchCase", () => {
    ...
    it("returns the correct value when a Valid is provided", () => {
      fc.assert(
        fc.property(fc.string(), s => {
          expect(
            V.Valid<[string], string>(s).matchCase({
    ...
    it("returns the correct value when a Invalid is provided", () => {
      fc.assert(
        fc.property(fc.string(), s => {
          expect(
            V.Invalid<[string], string>([s]).matchCase({
              invalid: ([x]) => x.length - 1,
              valid: x => x.length
            })
          ).toEqual(s.length - 1);
  ...
  describe("or", () => {
    it("Picks the first if Valid", () => {
      expect(simplify(V.Valid("foo").or(V.Valid("bar")))).toEqual(
        simplify(V.Valid("foo"))
      );
    });
    it("Picks the second if first Invalid", () => {
      expect(simplify(V.Invalid(["error"]).or(V.Valid("bar")))).toEqual(
        simplify(V.Valid("bar"))
      );
    });
    it("Picks the both if both Invalid", () => {
      expect(simplify(V.Invalid(["error1"]).or(V.Invalid(["error2"])))).toEqual(
        simplify(V.Invalid(["error2"]))
      );
    });
  });
  ...
  describe("replace", () => {
    it("Returns something if both are Valid", () => {
      expect(simplify(V.Valid("foo").replace(V.Valid("bar")))).toEqual(
        simplify(V.Valid("bar"))
      );
    });
    it("Returns Invalid if the second is Invalid", () => {
      expect(simplify(V.Valid("foo").replace(V.Invalid(["error"])))).toEqual(
        simplify(V.Invalid(["error"]))
      );
    });
    it("Returns Invalid if the first is Invalid", () => {
      expect(simplify(V.Invalid(["error"]).replace(V.Valid("bar")))).toEqual(
        simplify(V.Invalid(["error"]))
      );
    });
    it("Returns Invalid if both are Invalid", () => {
      expect(
        simplify(V.Invalid(["error1"]).replace(V.Invalid(["error2"])))
  ...
  describe("replacePure", () => {
    it("Replaces value if Valid", () => {
      expect(simplify(V.Valid("foo").replacePure(2))).toEqual(
        simplify(V.Valid(2))
      );
    });
    it("Returns Invalid if Invalid", () => {
      expect(simplify(V.Invalid(["error"]).replacePure(2))).toEqual(
        simplify(V.Invalid(["error"]))
      );
    });
  });
  ...
  describe("toEither", () => {
    it("Returns Just if Valid", () => {
      expect(simplify(V.Valid("foo").toEither())).toEqual(
        simplify(Right("foo"))
      );
    });
    it("Returns Nothing if Invalid", () => {
      expect(simplify(V.Invalid(["error"]).toEither())).toEqual(
        simplify(Left(["error"]))
      );
    });
  });
  ...
  describe("toMaybe", () => {
    ...
    it("Returns Nothing if Invalid", () => {
      expect(simplify(V.Invalid(["error"]).toMaybe())).toEqual(
        simplify(Nothing())
      );
    });
  });
  ...
  describe("voidOut", () => {
    it("Returns Invalid (s) for Invalid (s)", () => {
      expect(simplify(V.Invalid(["error"]).voidOut())).toEqual(
        simplify(V.Invalid(["error"]))
      );
    });

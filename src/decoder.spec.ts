...
prove<Equals<D.MapDecoder<any, string[]>, Array<D.Decoder<any, string>>>>(
  "proof"
);
  ...
  it("equals Invalid when any components have no value", () => {
    fc.assert(
      fc.property(
        ...
        fc.string(),
        fc.array(
          fc.oneof(fc.constant("bar"), fc.constant("baz"), fc.constant("qux")),
          1,
          3
        ),
    ...
    fc.assert(
      fc.property(fc.array(fc.integer()), (xs: number[]) => {
        const resultForM = simplify(
          D.forM(xs, x => D.constant(x.toString())).decode(null)
        );
        const resultMapM = simplify(
          D.mapM(x => D.constant(x.toString()), xs).decode(null)
        );
    ...
    fc.assert(
      fc.property(
        ...
        (a: number, b: boolean, c: string) => {
          expect(
            simplify(
              D.lift(f, D.constant(a), D.constant(b), D.constant(c)).decode(
                null
              )
            )
    ...
    fc.assert(
      fc.property(
        ...
        (a: number, b: boolean, c: string, empties: number[]) => {
          ...
          expect(
            simplify(
              D.lift(f, getArg(0, a), getArg(1, b), getArg(2, c)).decode(null)
            )
          ).toEqual(
            simplify(
              Invalid(
      ...
      fc.property(
        ...
        (xys: Array<[number, string]>) => {
          expect(
            simplify(
              D.mapAndUnzipWith(
                ([x, y]) => D.constant<[string, number]>([y, x]),
                xys
              ).decode(null)
            )
          ).toEqual(
            simplify(
              Valid(unzip(xys.map(([x, y]) => [y, x] as [string, number])))
            )
          );
        }
      )
  ...
  it("is equal to wrap for only Valid results", () => {
    fc.assert(
      fc.property(fc.array(fc.string()), (strs: string[]) => {
        expect(simplify(D.sequence(strs.map(D.constant)).decode(null))).toEqual(
          simplify(Valid(strs))
        );
      })
    );
      ...
      fc.property(
        ...
        (strs: string[], ns: number[]) => {
          expect(
            simplify(
              D.zipWithM(
                (str, n) => D.constant(str.length + n),
                strs,
                ns
              ).decode(null)
            )
          ).toEqual(
            simplify(Valid(strs.zipWith((str, n) => str.length + n, ns)))
          );
        }
      )
    );
  ...
  it("fails invalid input", () => {
    fc.assert(
      fc.property(
        ...
        (input: any) => {
          expect(simplify(D.boolean.decode(input))).toEqual(
            simplify(Invalid({ $: "Expected a boolean" }))
          );
        }
      )
    );
  ...
  it("decodes numbers", () => {
    fc.assert(
      fc.property(
        ...
        (input: any) => {
          expect(simplify(D.number.decode(input))).toEqual(
            simplify(Valid(input))
          );
        }
      )
    );
  ...
  it("fails invalid input", () => {
    fc.assert(
      fc.property(
        ...
        (input: any) => {
          expect(simplify(D.number.decode(input))).toEqual(
            simplify(Invalid({ $: "Expected a number" }))
          );
        }
      )
    );
  ...
  it("decodes strings", () => {
    fc.assert(
      fc.property(
        ...
        (input: any) => {
          expect(simplify(D.string.decode(input))).toEqual(
            simplify(Valid(input))
          );
        }
      )
  ...
  it("fails invalid input", () => {
    fc.assert(
      fc.property(
        ...
        (input: any) => {
          expect(simplify(D.string.decode(input))).toEqual(
            simplify(Invalid({ $: "Expected a string" }))
          );
        }
      )
    );
...
describe("array", () => {
  it("decodes arrays when decoder succeeds", () => {
    fc.assert(
      fc.property(fc.array(fc.anything()), input => {
        expect(simplify(D.array(D.id).decode(input))).toEqual(
          simplify(Valid(input))
        );
      })
    );
  ...
  it("fails to decode arrays when decoder fails", () => {
    fc.assert(
      fc.property(fc.array(fc.anything(), 1, 10), input => {
        expect(
          simplify(D.array(D.constantFailure({ $: "error" })).decode(input))
        ).toEqual(
          simplify(
            Invalid(
              objectFromEntries(input.map((_: any, i) => [`[${i}]`, "error"]))
            )
          )
        );
      })
  ...
  it("fails invalid input", () => {
    fc.assert(
      fc.property(
        ...
        (input: any) => {
          expect(simplify(D.array(D.id).decode(input))).toEqual(
            simplify(Invalid({ $: "Expected an array" }))
          );
        }
      )
    );
...
describe("$case", () => {
  it("decodes items of the correct case", () => {
    const decoder = D.$case(
      "Foo",
      D.build<{ bar: string }>({ bar: D.property("bar", D.string) })
    );
    expect(simplify(decoder.decode({ __case: "Foo", bar: "foo" }))).toEqual(
      simplify(Valid({ __case: "Foo", bar: "foo" }))
    );
  });
  ...
  it("fails items of the correct case", () => {
    ...
    expect(simplify(decoder.decode({ bar: "foo" }))).toEqual(
      simplify(Invalid({ __case: "Expected Foo" }))
    );
  });
});
...
describe("oneOf", () => {
  it("decodes allowed items", () => {
    fc.assert(
      fc.property(
        fc.oneof<string | number | boolean>(
          fc.constant("foo"),
          fc.integer(),
          fc.boolean()
        ),
    ...
    fc.assert(
      fc.property(
        fc
          .anything()
          .filter(
            x => x !== "foo" && typeof x !== "number" && typeof x !== "boolean"
          ),
        input => {
          expect(
            simplify(
...
describe("optional", () => {
  it("decodes null", () => {
    expect(
      simplify(D.optional(D.constantFailure({ $: "Fail" })).decode(null))
    ).toEqual(simplify(Valid(Nothing())));
  });
  it("decodes undefined", () => {
    expect(
      simplify(D.optional(D.constantFailure({ $: "Fail" })).decode(undefined))
    ).toEqual(simplify(Valid(Nothing())));
  });
  it("fails if decoder fails", () => {
    fc.assert(
      fc.property(
        ...
        (input: any) => {
          expect(
            simplify(D.optional(D.constantFailure({ $: "Fail" })).decode(input))
  ...
  it("succeeds if decoder succeeds", () => {
    fc.assert(
      fc.property(
        fc.anything().filter(x => x != null),
        (input: any) => {
          expect(simplify(D.optional(D.id).decode(input))).toEqual(
            simplify(Valid(Just(input)))
          );
        }
      )
    );
...
describe("property", () => {
  it("fails if decoder fails", () => {
    fc.assert(
      fc.property(
        fc
          .object({ key: fc.constant("foo") })
          .filter(x => Object.keys(x).length > 0),
        input => {
          expect(
            simplify(
              D.property("foo", D.constantFailure({ $: "Fail" }) as any).decode(
                input
              )
            )
    ...
    fc.assert(
      fc.property(
        fc
          ...
          .filter(x => Object.keys(x).length > 0),
        input => {
          expect(simplify(D.property("foo", D.id).decode(input))).toEqual(
            simplify(Valid(input.foo))
          );
        }
      )
    );
  ...
  it("fails if property not found and property is required", () => {
    fc.assert(
      fc.property(
        ...
        input => {
          expect(simplify(D.property("foo", D.string).decode(input))).toEqual(
            simplify(Invalid({ foo: "Expected a string" }))
          );
        }
      )
    );
  ...
  it("fails if property not found and property is optional", () => {
    fc.assert(
      fc.property(
        ...
        input => {
          expect(
            simplify(D.property("foo", D.optional(D.string)).decode(input))
          ).toEqual(simplify(Valid(Nothing())));
        }
      )
    );
  ...
  it("decodes tuples when decoders succeed", () => {
    fc.assert(
      fc.property(
        ...
        input => {
          expect(simplify(D.tuple(D.id, D.id, D.id).decode(input))).toEqual(
            simplify(Valid(input))
          );
        }
      )
      ...
      fc.property(
        ...
        input => {
          expect(
            ...
            simplify(
              Invalid({ "[0]": "error1", "[1]": "error2", "[2]": "error3" })
            )
          );
        }
      )
  ...
  it("fails invalid input", () => {
    fc.assert(
      fc.property(
        ...
        input => {
          expect(simplify(D.tuple(D.id, D.id, D.id).decode(input))).toEqual(
            simplify(Invalid({ $: "Expected an array of length 3" }))
          );
        }
      )
    );
...
describe("Decoder", () => {
  describe("map", () => {
    ...
    it("Skips the callback on empty", () => {
      ...
      expect(
        simplify(
          D.constantFailure<string>({ key: "error" })
            .map(k)
            .decode(null)
        )
      ).toEqual(simplify(Invalid({ key: "error" })));
    });
  });
  ...
  describe("or", () => {
    it("Picks the first if Valid", () => {
      ...
      ).toEqual(simplify(Valid("foo")));
    });
    it("Picks the second if first Invalid", () => {
      expect(
        simplify(
          D.constantFailure({ key: "error" })
            .or(D.constant("bar"))
            .decode(null)
        )
    ...
    it("Picks the second if both Invalid", () => {
      expect(
        simplify(
          D.constantFailure({ key1: "error" })
            .or(D.constantFailure({ key2: "error" }))
            .decode(null)
        )
      ).toEqual(simplify(Invalid({ key2: "error" })));
    });
  });
  ...
  describe("replace", () => {
    ...
    it("Returns Invalid if the second is Invalid", () => {
      expect(
        simplify(
          D.constant("foo")
            .replace(D.constantFailure({ key: "error" }))
            .decode(null)
        )
      ).toEqual(simplify(Invalid({ key: "error" })));
    });
    ...
    it("Returns Invalid if both are Invalid", () => {
      ...
      ).toEqual(simplify(Invalid({ key1: "error", key2: "error" })));
    });
  });
...
describe("combinations", () => {
  ...
  it("composes results", () => {
    expect(
      simplify(
        parentDecoder.decode({
          children: [{ foo: "baz" }, { foo: "qux", bar: 24 }]
        })
      )
    ).toEqual(
      simplify(
  ...
  it("composes errors", () => {
    expect(
      simplify(
        parentDecoder.decode({ children: [{ foo: true }, { bar: false }] })
      )
    ).toEqual(
      simplify(
        Invalid({

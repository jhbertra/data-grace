 ...
 * 
 *      // Write a type test
 *      type PropEquality = FooDecoders extends { bar: Decoder<string, number>, baz: Decoder<string, string> } ? any : never;
...
export const date: Decoder<any, Date> = Decoder((value: any) => {
    if (value instanceof Date) {
        ...
        return parsed === NaN
            ? V.Invalid(<DecodeError>{ "$": "Expected a date" })
...
export const boolean: Decoder<any, boolean> = Decoder(
    (value: any) => typeof (value) === "boolean" ? V.Valid(value) : V.Invalid(<DecodeError>{ "$": "Expected a boolean" }));
...
export const number: Decoder<any, number> = Decoder(
    (value: any) => typeof (value) === "number" ? V.Valid(value) : V.Invalid(<DecodeError>{ "$": "Expected a number" }));
...
export const string: Decoder<any, string> = Decoder(
    (value: any) => typeof (value) === "string" ? V.Valid(value) : V.Invalid(<DecodeError>{ "$": "Expected a string" }));
...
export function array<T>(convert: Decoder<any, T>): Decoder<any, T[]> {
    return Decoder(value => Array.isArray(value)
        ...
        : V.Invalid(<DecodeError>{ "$": "Expected an array" }));
}
    ...
    return Decoder(value => {
        ...
        return found
            ...
            : V.Invalid(choices.length === 0
                ? <DecodeError>{ "$": "There are no valid options to choose from" }
                : <DecodeError>{ "$": `Valid options: ${choices.join(" | ")}` });
    });
}
...
export function object<T extends object>(convert: Decoder<object, T>): Decoder<object, T> {
    return Decoder(
        value => typeof (value) === "object" && value !== null ? convert.decode(value) : V.Invalid(<DecodeError>{ "$": "Expected an object" }));
}
...
export function property<T>(
    ...
    return Decoder(obj => obj.hasOwnProperty(name)
        ? convert.decode((<any>obj)[name]).mapError(error => prefixError(name, error))
        : V.Invalid(<DecodeError>{ [name]: "Required" }));
}
...
export function tuple<T extends any[]>(...converters: { [K in keyof T]: Decoder<any, T[K]> }): Decoder<any, T> {
    return Decoder(value => Array.isArray(value)
        ? value.length === converters.length
            ...
            : V.Invalid(<DecodeError>{ "$": `Expected an array of length ${converters.length}` })
        : V.Invalid(<DecodeError>{ "$": `Expected an array of length ${converters.length}` }));
}
 ...
 *      fooDecoder.decode({ bar: "eek", baz: false }); // Valid ({ "bar": "eek", "baz": { "tag": "Just", "value": false } })
 *      fooDecoder.encode({ bar: "eek", baz: Just(false) }); // { bar: "eek", baz: false }
 */
export function liftO<T>(spec: MapDecoder<object, T>): Decoder<object, T> {
...
export function sequence<TIn, A>(das: Decoder<TIn, A>[]): Decoder<TIn, A[]> {
    return mapM(id, das);
}

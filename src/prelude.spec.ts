...
import "./array";
import { constant, curry, id, objectFromEntries, objectToEntries, pipe, pipeWith } from "./prelude";

describe("constant", () => {
    it("Returns the same input always", () => {
        fc.assert(fc.property(fc.anything(), fc.anything(), (input, x) => expect(constant(input)(x)).toEqual(input)));
    });
});
...
describe("curry", () => {
    ...
    it("Returns the same as the uncurried function", () => {
        fc.assert(fc.property(
            ...
            fc.integer(),
            fc.boolean(),
            (a, b, c) => expect(curry(f)(a)(b)(c)).toEqual(f(a, b, c))));
    });
});

 ...
 */
function arrayToValidation<A extends object | any[], B>(arr: B[], a: A): Validation<A, B> {
 ...
 */
function maybeToValidation<A extends object | any[], B>(maybe: Maybe<B>, a: A): Validation<A, B> {
    return maybe.matchCase({ just: (b: B) => Valid(b), nothing: () => Invalid(a)});
}
 ...
 */
function eitherToValidation<A extends object | any[], B>(either: Either<A, B>): Validation<A, B> {
    return either.matchCase({ left: (a: A) =>  Invalid(a), right: (b: B) => Valid(b)  });
}
 ...
 */
function successful<A extends object | any[], B>(vs: Array<Validation<A, B>>): B[] {
    const result: B[] = [];
    for (const validation of vs) {

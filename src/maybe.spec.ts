        ...
        fc.assert(
            fc.property(
                ...
                (a: number, b: boolean, c: string) => {
                    expect(simplify(M.lift(f, M.Just(a), M.Just(b), M.Just(c)))).toEqual(simplify(M.Just(f(a, b, c))));
                }));
    });
        ...
        fc.assert(
            fc.property(
                ...
                (a: number, b: boolean, c: string, empties: number[]) => {
                    ...
                    expect(simplify(M.lift(f, getArg(0, a), getArg(1, b), getArg(2, c))))
                        .toEqual(simplify(M.Nothing()));
                }));
    });
});
        ...
        fc.assert(
            fc.property(
                ...
                (xys: Array<[number, string]>) => {
                    expect(simplify(M.mapAndUnzipWith(([x, y]) => M.Just<[string, number]>([y, x]), xys)))
        ...
        fc.assert(
            fc.property(
                ...
                ([strs, empties]) => {
                    ...
                    expect(M.mapMaybe((str) => predicate(str) ? M.Just(str.length) : M.Nothing(), strs))
    ...
    it("is equal to reduce + wrap for only pure results", () => {
        fc.assert(
            fc.property(
                fc.array(fc.string()),
                (strs: string[]) => {
                    expect(simplify(M.reduceM((state, str) => M.Just(state.concat(str)), "", strs)))
                        .toEqual(simplify(M.Just(strs.reduce((state, str) => state.concat(str), ""))));
                }));
    });
    it("is equal to Nothing for any empty results", () => {
        fc.assert(
    ...
    it("is equal to Nothing for any empty results", () => {
        fc.assert(
            fc.property(
                ...
                ([strs, empties]) => {
                    const predicate = ([s, i]: [string, number]) => empties.find((x) => x === i) == null;
                    expect(simplify(M.sequence(strs.map((str) => predicate(str) ? M.Just(str[0]) : M.Nothing()))))
                        .toEqual(simplify(M.Nothing()));
                }));
    });
});
...
describe("toMaybe", () => {
    ...
    it("produces a value for values", () => {
        fc.assert(fc.property(
            fc.integer(),
            (n: number) => expect(simplify(M.toMaybe(n))).toEqual(simplify(M.Just(n)))));
    });
});
...
describe("IMaybe", () => {
    ...
    describe("filter", () => {
        it("Passes the value into predicate", () => {
            fc.assert(fc.property(fc.string(), (s) => { M.Just(s).filter((x) => !!expect(x).toEqual(s)); }));
        });
